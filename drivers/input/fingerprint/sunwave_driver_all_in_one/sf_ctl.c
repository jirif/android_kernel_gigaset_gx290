...
#ifndef CONFIG_NAVIGATION_FUNCTION  //prize-chj-20190528 Fingerprint navigation
        case SF_KEY_ENTER:
            key_code = KEY_ENTER;
            break;
        case SF_KEY_UP:
...
static long sf_ctl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        ...
        case SF_IOC_SET_LIB_VERSION: {
			...
			/* prize-wyq, sunwave fingerprint info, 20190420-start */
			#if defined(CONFIG_PRIZE_HARDWARE_INFO)
				snprintf(current_fingerprint_info.chip,32,"%s %s",sf_hw_ver.sunwave_id,sf_hw_ver.firmware);
				snprintf(current_fingerprint_info.id,32,"ca:%s",sf_hw_ver.ca_version);// sf_hw_ver.ta_version
				strcpy(current_fingerprint_info.vendor,"sunwave");
...
/* prize-wyq, sunwave fingerprint info, 20190420-start */
#if defined(CONFIG_PRIZE_HARDWARE_INFO)
	sprintf(current_fingerprint_info.chip,"sunwave_fp");
	sprintf(current_fingerprint_info.id,"%s",sf_ctl_get_version());
	strcpy(current_fingerprint_info.vendor,"sunwave");
	strcpy(current_fingerprint_info.more,"fingerprint");
#endif
/* prize-wyq, sunwave fingerprint info, 20190420-end */
...
static int sf_ctl_init_input(void)
{
	...
	#ifdef CONFIG_NAVIGATION_FUNCTION //prize-chj-20190528 Fingerprint navigation
	__set_bit(KEY_F1,     sf_ctl_dev.input->keybit);
    __set_bit(KEY_F2,     sf_ctl_dev.input->keybit);
    __set_bit(KEY_F3,     sf_ctl_dev.input->keybit);
    __set_bit(KEY_F4,     sf_ctl_dev.input->keybit);
    __set_bit(KEY_F12,    sf_ctl_dev.input->keybit);
	#endif
